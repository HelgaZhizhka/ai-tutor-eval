...
import { writeRawResult, writeSummary } from "./report.js";
...
async function main(): Promise<void> {
  ...
  console.table(models.map((model) => ({
    ...
    estimated_cost_usd: estimateCostUsd(pricing.get(model) ?? { prompt: 0, completion: 0 }, selected.length).toFixed(4)
  })));
  console.log(`Profile=${profile}; calls=${calls}; total estimated maximum=$${estimate.toFixed(4)}; cap=$${limit.toFixed(2)}`);
