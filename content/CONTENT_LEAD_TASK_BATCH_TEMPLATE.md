# Olympiad Academy — Grade 5 Mathematics: Approved Task Batch

_A Content Lead-friendly template. This is not a YAML or coding file._

## How to use this template

Please copy the task section below for every task in the batch. A batch of five fully reviewed tasks is a good starting point for the first meaningful model comparison.

For the current learner-facing MVP, please provide Uzbek in Latin script. If a task will later be shown in another language, that version needs its own human review.

If a field needs more work, please write `Pending review` rather than guessing. We will not add a task to the active AI evaluation set until it is approved.

The technical team will transfer the approved information into the evaluation repository. You do not need to edit YAML files or use Git.

---

## Task ID — Short topic title

**Review status:** approved  
**Reviewed by:** Name  
**Review date:** YYYY-MM-DD  
**Language:** Uzbek Latin
**Grade:** 5  
**Topic:**  
**Difficulty:** 1 — Fluency / 2 — Standard / 3 — Challenge  
**Skills:**  
**Prerequisite skills:**  

### Student-facing statement

Write the exact task as the student should see it.

### Canonical answer

Write the expected final answer with the appropriate unit or form.

### Other accepted answer forms

List mathematically correct alternatives that should be accepted. For example: `14`, `14 buses`, `14 minibuses`.

### Teacher-approved solution steps

1. 
2. 
3. 

### Hint ladder

- **Hint 1 — smallest nudge:**
- **Hint 2 — stronger guidance:**
- **Hint 3 — final non-solution hint:**

For the current Demo Day flow, all three hints are static teacher-approved
in-attempt support. The static `solution_steps` are the Full Walkthrough shown
only after Hint 3 if the learner is still stuck.

### Common misconceptions and the appropriate tutor response _(optional)_

Complete this section only if the task is selected for a future Ask Tutor
evaluation and there is a likely, well-understood wrong turn that the AI should
handle. It is not required for every task in the static Hint 1 → Hint 2 → Hint
3 → Full Walkthrough flow.

| Misconception / likely wrong turn | Example student answer or reasoning | What should Ask Tutor do next? |
| --- | --- | --- |
|  |  |  |
|  |  |  |
|  |  |  |

### Important alternative correct approaches _(optional)_

If there is a likely valid method that differs from the canonical solution — for example drawing, grouping, a table or systematic search — describe how the tutor should recognise it without forcing the student back to the canonical method.

### Checking guidance _(optional)_

What is the key mathematical step that shows understanding? What should be accepted as correct, incomplete or incorrect?

### Source and use note

For an original task, write for example: `Original task prepared for Olympiad Academy pilot; approved for project use.`

For an adapted task, note the original source and any use restriction known to the Content Lead.

### Notes for the team _(optional)_

Anything the frontend, product or technical team should know about this task.

---

## What happens after the task is approved

1. The technical team transfers the task into the private content workspace.
2. The evaluation owner drafts only the few student scenarios needed for the selected AI mode.
3. The Content Lead reviews the expected tutor behaviour in those scenarios.
4. Only then do the scenarios enter an active model-evaluation set.
